...
/**
 * 🆕 NEW FEATURE
 * List all generated certificates for an event
 * (for organiser to view/download)
 */
const listCertificatesForEvent = async (req, res) => {
  try {
    ...
    // If no certificates generated yet
    if (!fs.existsSync(certDir)) {
      return res.json([]);
    }

    const files = fs.readdirSync(certDir).filter(file =>
      file.endsWith(".pdf")
    );
